 ...
 * 기본적인 정렬들
 *  1. 버블 : 인접한 두개의 데이터를 비교해가면서 정렬을 진행하는 방법
 *  2. 선택 : 값을 선택하고, 배열을 탐색하여 선택한 위치에 적합한 값을 찾아 두 값을 서로 교체하는 방법 이를 반복하여 정렬
 *  3. 삽입 : 값을 선택하고, 배열을 탐색하면서 값을 한칸씩 밀어내면서 자기 위치를 찾아가는 방법
 *  4. 쉘 : 배열을 분할하여 각 부분별로 삽입정렬을 사용한방법 (분할정복 기법과 유사함)
 *
 *
 */
