...
public class Dijkstra {
	...
	public static void main(String[] args){
		...
		MinHeap h = new MinHeap(pointNum); // 각 정점까지의 거리를 정리하고 최소값을 얻어올 최소힙
		...
		while(!h.isEmpty()){
			...
			for(int i=0; i<weight.length; i++){
				if(weight[i][0] == temp.d){
					if(distance[weight[i][1]] == -1) { //방문하지 않았던 점이라면
						distance[weight[i][1]] = temp.bcost + weight[i][2]; // 해당정점에 이르는 비용을 저장
						h.put(new Node(temp.bcost+weight[i][2], weight[i][1]));
						// 해당정점의 이전까지의 비용(temp.bcost) + 그 점에서 자신으로의 비용( weight[i][2]), 정점 자신의번호
						path[weight[i][1]] = weight[i][0]; // 해당 정점에 이르는 경로 path[목표정점] = 이전 정점
					}
					else{ //방문 했던 점이라면 비용을 비교해서 변경
						if( distance[weight[i][1]] < temp.bcost) distance[weight[i][1]] = temp.bcost;

					}
					h.put(new Node(temp.bcost+weight[i][2], weight[i][1]));
					// 해당정점의 이전까지의 비용(temp.bcost) + 그 점에서 자신으로의 비용( weight[i][2]), 자기 정점번호
				}
			}
